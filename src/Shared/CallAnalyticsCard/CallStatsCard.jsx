...
const Icon = ({ iconSrc, label }) => {
  return iconSrc ? (
    <img src={iconSrc} alt={label} className="w-5 h-5" title={label} />
  ) : (
    <div className="w-5 h-5 bg-gray-300 rounded" title={label}></div>
  );
};
const CallStatCard = ({ title, iconKey, value, subtitle, subtitleColor, borderColor }) => (
    ...
    <div className="flex justify-between items-center mb-2">
      ...
      <Icon iconSrc={iconKey} label={title} /> {/* <-- pass the actual image */}
    </div>
